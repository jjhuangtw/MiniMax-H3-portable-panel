...
def lora_choices():
    ...
    files = list_model_files("loras", "LoraLoaderModelOnly", "lora_name")
    return [NO_LORA] + [f for f in files
                        if os.path.basename(f) not in turbo
                        and not f.replace("\\", "/").startswith("krea2/")]
...
with gr.Blocks(title="MiniMax H3 Portable - RTX 4090") as demo:
    ...
    with gr.Accordion("🧩 模型設定（編碼器・LoRA・擴散模型，共用）", open=False):
        with gr.Row():
            ...
            model_lora = gr.Dropdown(label="額外 LoRA（疊加在 Turbo LoRA 之後）", choices=lora_choices(), value=NO_LORA, scale=4)
        ...
        model_refresh.click(refresh_model_choices, None, [model_encoder, model_lora, model_fl2va, model_ref2va], queue=False)
    model_inputs = [model_encoder, model_lora, model_lora_strength, model_fl2va, model_ref2va]
    ...
    with gr.Tabs():
        with gr.Tab("🎬 文生"):
            ...
            t2v_btn.click(
                fn=lambda p, r, d, tb, s, hd, enc, lora, ls, fl2, r2v: execute_generation(p, r, d, tb, s, text_encoder=enc, lora_name=lora, lora_strength=ls, fl2va_model=fl2, ref2va_model=r2v, hd=hd),
        ...
        with gr.Tab("🖼️ 首尾幀"):
            ...
            i2v_btn.click(
                fn=lambda p, r, d, tb, s, f, l, hd, ki, enc, lora, ls, fl2, r2v: execute_generation(p, r, d, tb, s, f, l, text_encoder=enc, lora_name=lora, lora_strength=ls, fl2va_model=fl2, ref2va_model=r2v, hd=hd, keyframe_instruction=ki),
        ...
        with gr.Tab("🎞️ 參考"):
            ...
            ref_btn.click(
                fn=lambda p, r, d, tb, s, imgs, vids, va, auds, mode, sch, enc, lora, ls, fl2, r2v: execute_ref_generation(
        ...
        with gr.Tab("📼 長片"):
            ...
            long_btn.click(
                fn=lambda p, sp, tot, seg, r, s, img, sch, md, enc, lora, ls, fl2, r2v: execute_long_generation(p, sp, tot, seg, r, s, img, mode=md, scheduler=sch, text_encoder=enc, lora_name=lora, lora_strength=ls, fl2va_model=fl2, ref2va_model=r2v),
        ...
        with gr.Tab("🎥 攝影機"):
            ...
            camera_btn.click(
                lambda p, r, tb, s, image, state, enc, lora, ls, fl2, r2v: execute_generation(p, r, state["frames"] / 24, tb, s, image, camera_state=state, text_encoder=enc, lora_name=lora, lora_strength=ls, fl2va_model=fl2, ref2va_model=r2v),
