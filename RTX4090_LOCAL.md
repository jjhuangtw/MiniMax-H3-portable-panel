...
- 參考影片上傳前以 ffmpeg 轉為 24 fps、**裁成與成片同長**（最長 362 幀，約 15.08 秒）、短邊不超過 720。H3 按 24 fps 讀取參考影格；每一幀參考影格都和成片一起運算，超出成片長度的部分只會白白增加負擔。
- 後端以 `--disable-smart-memory` 啟動（2026-09-26）：否則 32B 文字編碼器在採樣時仍佔著約 11 GB 顯存，帶長參考影片的 Ref2VA 會把主幹擠到系統記憶體、停在第 0 步。實測 864×480、成片 11 秒＋參考影片 11 秒：原設定 5 分鐘以上仍在第 0 步（GPU 111 W）；加上此參數 7.5 分鐘完成（每步約 75 秒、445 W）。4 秒單圖 Ref2VA 反而從 67 秒變 43～54 秒。
- 成片＋參考影片的「像素×幀」超過約 1.5 億時，面板會先提醒預估時間（以上面的實測值換算）。
